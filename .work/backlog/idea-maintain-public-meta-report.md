...
# Maintain & improve the public the local meta-vs-global meta report

A public shareable report was published to share back with the the local meta tournament organizer
...
- **Dev copy / build:** `legacy-engine/decks/local-vs-global-meta-report.html` (gitignored). Deploy flow + data provenance captured in the `local-meta-public-report` memory.
...
- **Generalize beyond the local meta.** If other organizers share local data, the same engine-side machinery (local field file + global corpus) produces a report for any meta — the public repo can host several.

...
Engine-built decks for both followed metas: (1) Dimir Tempo tuned for the Boulder field,
...
`whattoplay` best-pick archetype for Boulder, (4) best-pick for online — each generated via
...
- Metas: Boulder = `decks/boulder-field-since-518.txt`; online = `provenance='online'`,
  current-regime window. Prior best-deck context: D&T topped both lenses on the regime-clean
  Boulder field (2026-06-27 session; re-run, don't assume).
...
Best-picks: **Painter** (Boulder, S*=0.528 Q0.25-ranked lean) and **Doomsday** (online,
S*=0.580, cov≈0 caveat) — no Dimir collision, so the collision rule stayed dormant.
Deliverables: decks/meta-{boulder,online}-{dimir-tempo,bestpick-*}.txt +

...
Fresh optimized sideboard for the maintainer's Dimir Tempo (Build B, `decks/dimir-tempo-current.txt`;
...
they differ: **Board A** unconstrained — may include cards the maintainer doesn't own, and those
...
- local field = `decks/local-field-since-518.txt`; online = `provenance='online'`,
  current-regime window.

## Design decisions (feature-design, 2026-07-04, autopilot)

- **Maindeck input**: `decks/dimir-tempo-current.txt` (= Build B, the maintainer's committed 75).
...
- **Primary lens = the local meta** (the maintainer plays paper the local meta); the online-lens board is a
...
2. **Board A (the local meta lens)** — `advise sideboard` vs local field with `--collection`,
...
4. **Online-lens variant** — same solve vs the online-provenance field; report only the
   DIFF vs the the local meta board (venue-divergence presentation, never blended). AC: diff

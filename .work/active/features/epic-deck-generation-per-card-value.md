...
- Parent epic: `epic-deck-generation`. This is the new prerequisite that the parent's `## Decomposition`
  flagged as "needs a per-card win-rate match-results extension" — now un-deferred per the maintainer's 2026-05-31
  decision.
...
Gives the maintainer a way to eyeball the (correlational) numbers on real data — auditability for "never fabricate".
**Acceptance Criteria**:

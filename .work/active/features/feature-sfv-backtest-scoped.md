...
1. **Filter decks by field-archetype membership, at the TOURNAMENT level (chosen).** For
   ...
   6/8 Reanimator does not represent a local field, so its top-finishing the local meta deck's
   ...
   to the local meta" without needing a ban-regime time default (see the window sub-decision
   below).
...
tournament (6/8 Reanimator, 2/8 the local meta, distinct sideboard signal "Grafdigger's Cage").

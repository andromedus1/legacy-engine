...
real `data/legacy.duckdb` + a real deck (`decks/doomsday-tempo-local.txt`) and the live the local meta
field snapshot (`decks/local-field-current.txt`): renders cleanly, all reference-archetype
confidence tiers show `speculative` (consistent with the documented ~36%-of-matchups-thin the local meta

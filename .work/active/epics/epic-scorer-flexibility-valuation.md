...
flexible interaction**, and its own backtest surface caught it: on Andrew's real Dimir Tempo deck vs
the 107-player Boulder field, `advise backtest` (670 top-finisher boards, established tier) shows
...
scored away. Re-run on Andrew's Dimir Tempo board + Boulder field as the concrete regression case.
...
- **Backtest scope**: **enhance to field/window-scoped in this epic** — validation must be Boulder-specific, not global-Dimir polluted by graveyard-meta tech, for an honest acceptance gate.

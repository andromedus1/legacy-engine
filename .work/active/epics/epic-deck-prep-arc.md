...
the maintainer's queued arc (2026-07-04, direction locked via an only-questions round): a five-stage
...
- **"Build 2 decks per meta" = 4 lists**: BOTH Dimir Tempo tuned per meta AND the engine's
  `whattoplay` best-pick per meta. Metas: local paper field
  (`decks/local-field-since-518.txt`) and online (MTGO-provenance current-regime field).

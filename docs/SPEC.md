...
decisions:
  ...
  - "STABLE ERAS: per-entity era boundaries detected from the corpus (change-point ensemble + fleet FDR + era floors), persisted with attribution and a drift alarm (eras run|list|explain|confirm), and consumed as the DEFAULT window everywhere (adaptive matrix, detection-derived global field era, consensus/card-frequency, era-default discovery pools with Gate C); matchup cells carry hierarchical + cross-era priors with prior_source labels. Byte-identical ban-only fallback when no era data exists."
  - "SUPERARCHETYPES: a third taxonomy level ABOVE parent archetype — strategy clusters derived from staple-stripped maindeck core sets (Jaccard + average-linkage agglomerative, AU-bootstrap cut) with a curated override layer, persisted as a registry by an offline pass. A subject's record against a cluster is a random-effects (DerSimonian-Laird) pooled proportion whose effective sample n_eff — not raw pooled n — feeds tier_for_sample and the display gate, gated on concentration (m_eff = 1/HHI >= 2.0, max member share <= 0.60) and heterogeneity (I-squared bands, with I-squared treated as ONE-SIDED evidence). Cells fall back archetype -> superarchetype per cell, labeled with a provenance chip; intra-cluster matches count but carry an intra-cluster flag."
created: 2026-05-29
updated: 2026-07-31
related:
...
- **[Building] Superarchetype layer (strategy clusters)** — the third taxonomy level, above parent archetype. An offline `superarchetype` pass clusters archetypes by **maindeck core set** (cards at >=50% inclusion) **minus format staples** (cards core to >=30% of the cluster-defining archetypes — hard removal, since soft TF-IDF down-weighting fuses half the field into one "plays blue" cluster), using **Jaccard dissimilarity + average-linkage agglomerative** clustering cut at the deepest height whose branches clear a multiscale-bootstrap **AU p-value >= 0.95**. Archetypes with >=30 decks and >=8 core cards may **define** a cluster; the long tail is **assigned** by nearest centroid; camps inherit their parent's cluster; a curated override layer wins by key and records the derived assignment it replaced. A subject's win rate against a cluster is a **random-effects (DerSimonian-Laird) inverse-variance pooled proportion** over the member cells, and the **`n_eff`** implied by its random-effects variance — never the raw pooled n — is what reaches `tier_for_sample()` and the display gate, so heterogeneity cannot buy a tier. Two gates plus two guards decide whether the pooled number may be claimed at all: concentration (`m_eff = 1/HHI >= 2.0` AND max member share <= 0.60; below that the cell is still served but labeled `dominated by <member>`), heterogeneity (I² <= 0.40 pool freely / 0.40-0.75 label `heterogeneous pool` / > 0.75 **refuse** the pooled number and show the member split), a direction/spread guard, and a minimum-computability rule. The cluster cell is a new rung in the shrinkage chain between the leave-camp-out parent cell and the subject marginal, computed leave-opponent-out; display walks the same ladder and takes the finest rung whose `n_eff` clears the gate, labeled with a provenance chip. Intra-cluster matches count and carry an intra-cluster flag (the exact self-mirror is excluded from the rate with its n reported). **I² is one-sided evidence** — a high value is a reliable stop, a low value is never a certificate of exchangeability, and that caveat is a UI-level obligation. Cluster membership is recomputed per window but cluster identity stays stable across windows (measured co-membership agreement 0.957 across the Flow State and Candelabra disturbances); churn is a diagnostic. See `docs/briefs/superarchetype-aggregation.md` and `epic-superarchetype-layer`.
...
| **EntityEras** | An entity's detected era history | Per archetype/camp: `stable_since` (None = full history is solid), boundaries (date, signals, magnitude, p-value, BH verdict, floor flag, attribution), drift-alarm state. Rebuildable from the corpus by `eras run`. |
...
- **Confidence-gated stats** — matchup cells and derived metrics carry sample size + confidence; low-n (n<100) flagged by default. Reuses edh-engine's confidence-metadata pattern. A **pooled** cell (superarchetype) gates on its **effective** sample `n_eff`, not the raw pooled n, so pooling members that disagree can never purchase a confidence tier.

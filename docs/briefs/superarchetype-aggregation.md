...
key_findings:
  ...
  - "HDBSCAN does not transfer: at ~30 cluster-defining archetypes there is no density contrast, and any archetype pushed into the noise class gets no superarchetype at all — a coverage failure aimed precisely at the thin archetypes the epic exists to serve. Average-linkage agglomerative on a precomputed Jaccard dissimilarity assigns every archetype, and pvclust-style multiscale-bootstrap AU p-values over the CARD features (not the archetypes) validate branches at small N."
  ...
  - "Random-effects inverse-variance pooling is the estimator that self-degrades correctly: with tau-squared = 0 (58.7% of poolable cells measured exactly zero) it collapses to the intuitive pooled-counts answer, and as members disagree its weights flatten toward equality, automatically defusing the one-dominant-member problem instead of needing a separate rule."
...
| Algorithm | **Average-linkage agglomerative** on the precomputed dissimilarity `[sklearn-clustering]{13}`; every archetype gets a cluster (no noise class) |
| Cut selection | Branch-level **multiscale-bootstrap AU p-value >= 0.95** over resampled card features `[superarchetype-pvclust]{11}`, cross-checked with co-membership stability `[cluster-stability-review]{18}` |
...
| Estimator | **Random-effects (DerSimonian-Laird) inverse-variance pooled proportion** on continuity-corrected logits `[superarchetype-meta-pooling]{7}` |
| Sample gate | Tier and display gate read **`n_eff`** derived from the random-effects variance, never the raw pooled n |
| Concentration gate | **Effective members `m_eff` = 1/HHI >= 2.0** AND max member share <= 0.60 `[superarchetype-hhi]{8}` |
| Heterogeneity gate | **I-squared <= 0.40** pool freely; **0.40-0.75** pool with a `heterogeneous pool` label; **> 0.75** refuse the pooled number and show the member split `[superarchetype-cochrane-heterogeneity]{5}` |
...
reaches 359,933,112, yet only 348,498 unique compositions appeared", making the full pairwise table
unbuildable — and its remedy is structurally identical to this epic: "we propose a more manageable
...
- **Copy counts and abundance-aware distances matter where a split is a quantity difference**
...
- **Two-gate validation.** Statistical validity plus a domain read, with the domain read as final
  arbiter — certifying a cluster "requires some domain knowledge, and is therefore, hard to verify
  independently" `[kritschgau-hypergraph]{20}`. That is precisely why the epic's **curated override
  layer** is not optional (§3.5).
...
- **HDBSCAN as the primary algorithm.** Two independent reasons. (a) *No density contrast at this
  N.* HDBSCAN's parameters are defined relative to a population — `min_cluster_size` is "the
  smallest size grouping that you wish to consider a cluster" and larger `min_samples` means "more
  points will be declared as noise" `[hdbscan-docs]{14}`; with ~30 objects there is no setting that
  separates dense from sparse regions meaningfully. (b) *The noise class is a coverage bug here.*
...
- **The double-dipping hazard, in its camp-level form.** The camp layer risked forming groups from
  ...
  outcomes** (`rounds`) — two disjoint measurements — so the trap is largely side-stepped. It
  returns the moment anyone tunes the dendrogram cut to maximize coverage or minimize I-squared on
...
**Caution on internal validity indices.** The cophenetic correlation of the recommended
Jaccard/average clustering is 0.916 (complete linkage 0.887), while TF-IDF-cosine/average scores
**higher** at 0.944 — despite producing the useless 14-member blue mega-cluster. An internal index
...
linkage chains) or by the worst pair (complete linkage fragments). It accepts a precomputed
dissimilarity matrix, so Jaccard feeds it directly, and it assigns every archetype.
...
- **Primary — branch support by multiscale bootstrap.** Compute an AU (approximately unbiased)
  ...
  small N: the resampling is over features, not over the objects being clustered** — with ~30
  archetypes but hundreds of core cards, the bootstrap has plenty to work with. Accept the deepest
  cut at which every retained branch has AU >= 0.95.
- **Secondary — co-membership stability.** Represent the clustering as a binary co-membership matrix
  and require stability above 0.9 under perturbation `[cluster-stability-review]{18}`.
...
Two distinct populations, two rules (author's integration design, thresholds measured):
...
shell/color artifact, i.e. the failure the literature predicts when a statistical criterion is
allowed the final word `[kritschgau-hypergraph]{20}`. Curated entries win by key; derived fills gaps
...
`[superarchetype-gelman-multilevel]{1}`. Worse, at unequal member exposure it is subject to the
named reversal: a trend "appears in several groups of data but disappears or reverses when the
groups are combined", which happens "when there are large differences in the number of at bats"
`[superarchetype-simpsons-paradox]{10}`. That is not hypothetical here — see §6.3.
...
underlying our data" `[superarchetype-meta-heterogeneity]{6}`.
...
- **When members agree it IS the simple answer.** With `tau² = 0` the weights reduce to plain
  inverse variance, which for binomial cells is essentially size weighting — the intuitive
  pooled-counts number. Measured: **58.7% of poolable cells have I-squared exactly 0.000** (§6), so
...
- **It is the same shrinkage logic already in the codebase.** Shrinking many noisy estimates toward a
  common centre lowers total error whenever three or more quantities are estimated jointly
  `[superarchetype-james-stein]{3}` — with the caveat the project must respect in its UI: the
...
on the logit scale and `p̄` the pooled probability; clamp `n_eff <= Σ n_k`. When `tau² = 0` this
returns `Σ n_k` (the honest full pooled sample); as heterogeneity grows it falls below it. Feed
...
- `tau² = 0` (coherent cluster) → `s` is unbounded → clamp at **30**, tying the maximum influence of
  a superarchetype prior to exactly one displayable cell's worth of evidence (`DISPLAY_GATE_N`).
...
Both bounds are project-grounded rather than arbitrary, and both are auditable in the prior_source
label.
...
`HHI <= 0.50`) AND no single member supplies more than 60% of n.**
...
**The threshold is calibrated on measured data, not picked round.** Across multi-member pooled cells
with n>0: median HHI is exactly **0.500** (`m_eff` 2.00), p25 0.320, p75 0.722, and **46.0% exceed
0.50**. So the gate bisects the population — it is a live constraint, not a rubber stamp. Restricted
to the cells that pooling actually *unlocks* (n>=30), median HHI is 0.258 (`m_eff` 3.9) and
**7 of 25 (28%) fail the gate** — a targeted, meaningful minority.
...
Cochrane's published bands are "0% to 40%: might not be important; 30% to 60%: may represent moderate
heterogeneity; 50% to 90%: may represent substantial heterogeneity; 75% to 100%: considerable
heterogeneity" `[superarchetype-cochrane-heterogeneity]{5}`. Mapped onto this project's three-state
honesty vocabulary:
...
- **Minimum computability.** Q/I-squared require at least two members with n>=5. With fewer, no
  heterogeneity claim may be made in either direction; the cell falls back to the concentration
  labeling of §5.
...
The pooled cell **clears `DISPLAY_GATE_N`** and would render as a confident 66.7% edge. It is a lie:
the two members disagree by 52 percentage points, and one of them supplies 69% of the sample.
...
also the family where naive pooling is least safe against a specific subject, and all three
independent gates catch it. The correct surface behaviour is: no pooled number, show
...
is none."** Treat the gate as **one-sided evidence**: a high value is a reliable stop signal; a low
value is never a certificate of exchangeability. The consequence for the surface is that a pooled
...
- **New dependency surface is small.** `scipy.cluster.hierarchy` (linkage/fcluster/cophenet) covers
  the clustering and `scipy` is already a core dep; the AU p-value computation is a bootstrap loop in
  numpy (pvclust itself is R `[superarchetype-pvclust]{11}` — port the criterion, not the package);
  DerSimonian-Laird is a closed-form moment estimator, ~15 lines. No new heavyweight dependency, in
  contrast to the camp layer's `hdbscan`/`umap-learn`.
...
gated-and-labeled. Restricted-maximum-likelihood estimation of `tau²` is more accurate than
DerSimonian-Laird but iterative; DL's closed form is the right first move at these K. Opponent-side

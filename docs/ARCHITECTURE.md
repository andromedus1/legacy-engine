...
| `ranking_benchmark.py` | Future-only validation contracts: a preregistered ten-estimator registry with `production-ci-gated` as primary; cutoff-safe parent-only snapshots; retrospective-fixed-parent or dated contemporaneous taxonomy replay; hashed frozen predictions and held-out data; explicit support/censoring and player-sensitivity boundaries; proper scores (log loss, Brier, calibration) and decision endpoints (rank tau, top-three, regret). Evaluation is read-only and never tunes production. | advisory-methods |
| `workflows/ranking_benchmark.py` | Filesystem/DuckDB benchmark adapters: build raw pre-cutoff snapshots, recompute origin predictions, orient later outcomes, and compose immutable fold evaluation. Whole-date non-overlapping 28-day folds reset/truncate at B&R boundaries; external estimators are optional dated exact-mapped snapshots. | advisory-methods |

...
decisions:
  ...
  - "DECK RANKINGS PROJECTION: advisory/deck_ranking.py consumes typed matchup measurements and normalized field shares to compute conditional Beta posterior cells, full-field performance, the non-mirror floor, seeded intervals, coverage, and Pareto status. advisory/recent_field.py supplies the provisional 28-day published-list field; advisory/field_scenario.py validates an optional private override while retaining global candidate presence. The atomic publisher attaches scenario provenance/comparison, successive-snapshot changes, and parent/build diagnostics outside the ranking authority payload. Local strategic-plan shares remain visible, but projections are unavailable until composition-specific aggregates exist."
...
| `doomsday_variants.py` | Read-only specialized report projection: classifies mutually exclusive Doomsday mana/signature cohorts from exact 75s, separates non-League standings from uniquely resolved cutoff-compatible physical rounds, removes and renormalizes Doomsday opponent mass from the current global field once, and applies the shared ranking kernel with fixed Beta(1,1) cells and no parent borrowing. `scripts/refresh_doomsday_variant_rankings.py` manually publishes its atomic standalone HTML outside the scheduled refresh. | doomsday-variant-rankings |
...
| `field_scenario.py` | Validates private Deck Rankings fields through the existing field grammar, preserving source hash, supplied counts, declared effective concentration, fixed-share status, unknown mass, and warnings. It supplies scenario posterior weights separately from global current-corpus candidate presence. | advisory-methods |

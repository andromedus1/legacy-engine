...
| `variants.py` | Sub-archetype variant tagging: given a card-presence registry (e.g. "Bauble" → requires card X), classifies each decklist's variant tag. Drives `generate consensus --variant` and `report meta --by-variant`. | — |

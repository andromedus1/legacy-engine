...
| `players/effect.py` | Experimental future-only sensitivity: a separate three-estimator registry fits deterministic L2-pooled antisymmetric deck residuals, repeat-player intercepts, and separately gated player-by-parent familiarity around frozen production-CI log odds. Earlier-origin selection, outcome-blind scheduled forecasts, player-aware/masked/neutral estimands, cold-start/venue strata, and conjunction-only stop/go remain outside production; the maximum result is `candidate-for-promotion-study`. | advisory-methods |

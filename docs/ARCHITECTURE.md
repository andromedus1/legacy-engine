...
decisions:
  ...
  - "DECK RANKINGS PROJECTION: advisory/deck_ranking.py consumes typed matchup measurements and normalized field shares to compute conditional Beta posterior cell means, full-field performance, the non-mirror minimum floor, seeded draw intervals, evidence coverage, and Pareto status. Performance and floor have independent leaders, with performance breaking floor ties; the visible floor range names the toughest cell while the all-opponent minimum interval remains diagnostic. advisory/recent_field.py builds the half-open observed field with a provisional 28-day exponential half-life, published-list denominator, exact integer sightings, weighted counts, Kish ESS, source composition, camps, and movement. scripts/refresh_best_call_ranking.py::_publish_deck_rankings applies clean interval cells once as overrides while retaining each cell's actual prior_strength and provenance; scripts/refresh_decision_data.py publishes the default decks/deck-rankings.html artifact."
  - "SERVED DECK RANKINGS EVALUATION: scripts/evaluate_deck_rankings.py --served-model freezes cutoff-refitted raw snapshots and shared production projections for every declared retrospective fixed-parent origin before later outcomes are loaded. Fixed prior-strength scales 1/.5/2 and the conditional opponent-plan-prior-v1 challenger share a cell grid and outcome-blind card-metadata quarantine policy; scored evidence includes proper scores, support/calibration/reciprocity, paired event differences, ranking order, and named-floor follow-up. The experiment is pending and has no selection, apply, publication, or deployment path."
...
| `deck_ranking_projection.py` | Shared projection handoff used by publication and retrospective evaluation. It resolves the same era/fallback/explicit-override source once per cell, scales only effective Beta prior strength for fixed sensitivity runs, and can overlay a conditional challenger prior without changing direct wins/n or selected-source identity. Frozen rows retain original/effective prior strength, prior contribution, and complete source provenance. | advisory-methods |
...
| `workflows/deck_ranking_evaluation.py` | Retrospective served-model experiment adapter. For each declared origin it builds a raw pre-cutoff snapshot, refits the production Deck Rankings inputs under fixed parent taxonomy, freezes hashed predictions for scales 1/.5/2 plus `opponent-plan-prior-v1`, and only then loads later outcomes. Card availability is observed-by-cutoff where release dates are unavailable; one outcome-blind quarantine policy with fixed `.5%` deck/`2%` round ceilings and visible raw/retained ledgers applies to every method rather than repairing metadata. Evaluation reports log loss, Brier score, calibration, support strata, reciprocity, paired event differences, performance/floor order, and later evidence for named floors. It has no selection or publication path. | advisory-methods |
